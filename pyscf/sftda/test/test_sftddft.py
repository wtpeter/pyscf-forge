...
class KnownValues(unittest.TestCase):
    ...
    def test_hf_tddft(self):
        ...
        td = mf.TDDFT_SF().set(extype=0, collinear_samples=50, nstates=2, positive_eig_threshold=0.4).run()
        ...
        td = mf.TDDFT_SF().set(extype=1, collinear_samples=50, nstates=2, positive_eig_threshold=-0.35).run()
        self.assertTrue(np.all(td.converged))
        self.assertAlmostEqual(abs(td.e - ref).max(), 0, 4)
