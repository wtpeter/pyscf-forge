...
pyscf.sftda.TDDFT_SF/TDA_SF can be directly applied to ROKS objects,
...
sfd_tddft = sftda.TDDFT_SF(mf) # same as umf.TDDFT_SF()
sfd_tddft.nstates = 5
...
sfd_tda = sftda.TDA_SF(mf) # same as umf.TDA_SF()
sfd_tda.extype = 1
sfd_tda.nstates = 5
sfd_tda.collinear_samples = -1
sfd_tda.kernel()
sfd_tda.analyze()

print_header("VALIDATION: Explicit Casida Matrix Diagonalization")
a, b = sftda.uhf_sf.get_ab_sf(mf, collinear_samples=50)
A_baba, A_abab = a
B_baab, B_abba = b
...
fake_td = sftda.TDDFT_SF(mf)
fake_td.nstates = 5
fake_td.extype = 1
fake_td.collinear_samples = 50
